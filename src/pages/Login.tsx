...
import { LeafIcon, LockIcon, MailIcon, EyeIcon, EyeOffIcon } from 'lucide-react';
const Login = ({
  onLogin,
  theme = 'dark'
}) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      setIsLoading(true);
      // Call the login function passed from App.tsx
  ...
  return <div className={`min-h-screen w-full ${bgClass} flex items-center justify-center p-4 relative overflow-hidden ${isPageLoaded ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}>
      ...
      {/* Login container - improved accessibility and responsiveness */}
      <div className="relative z-10 w-full max-w-md">
        {/* Logo */}
        <div className="flex justify-center mb-6 md:mb-8">
          <div className="bg-gradient-to-r from-green-500 to-blue-500 rounded-full p-3 shadow-lg">
        ...
        <div className={`${theme === 'dark' ? 'bg-gray-800/60 backdrop-blur-md border-gray-700/50' : 'bg-white/80 backdrop-blur-md border-gray-200'} border rounded-xl md:rounded-2xl shadow-xl p-6 md:p-8`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} id="password" value={password} onChange={e => setPassword(e.target.value)} disabled={isLoading} autoComplete="current-password" className={`${theme === 'dark' ? 'bg-gray-900/50 border-gray-700 text-gray-100' : 'bg-white/90 border-gray-300 text-gray-900'} border text-sm rounded-lg block w-full pl-10 p-2.5 pr-10 focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500`} placeholder="••••••••" />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <input id="remember" type="checkbox" className={`w-4 h-4 ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-300'} rounded focus:ring-green-500 focus:ring-2`} />
                <label htmlFor="remember" className={`ml-2 text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                  Remember me
                </label>
              </div>
              <a href="#" className={`text-sm ${theme === 'dark' ? 'text-green-400' : 'text-green-600'} hover:underline focus:outline-none focus:underline`}>
                Forgot password?
              </a>
            </div>
            <button type="submit" disabled={isLoading} className={`w-full text-white bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600 font-medium rounded-lg text-sm px-5 py-3 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 shadow-md transition-all flex items-center justify-center ${isLoading ? 'opacity-80 cursor-not-allowed' : ''}`}>
              {isLoading ? <>
          ...
          <div className="mt-6 text-center">
            <p className={theme === 'dark' ? 'text-gray-400 text-sm' : 'text-gray-600 text-sm'}>
              ...
              <Link to="/signup" className={`${theme === 'dark' ? 'text-green-400' : 'text-green-600'} hover:underline focus:outline-none focus:underline`}>
                Create one
              </Link>
            </p>
          </div>
