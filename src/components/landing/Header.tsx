  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'py-2 bg-black/70 backdrop-blur-lg' : 'py-4 bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center group">
          ...
          rotate: 15
        }} className="bg-gradient-to-r from-green-500 to-blue-500 rounded-full w-10 h-10 flex items-center justify-center mr-2 shadow-lg shadow-green-500/20">
        ...
        <nav className="hidden md:flex items-center space-x-1">
          {['About', 'Features', 'Pricing', 'Contact'].map(item => <a key={item} href={`#${item.toLowerCase()}`} className="px-4 py-2 text-gray-300 hover:text-white relative group">
              {item}
      ...
      <AnimatePresence>
        ...
        duration: 0.3
            ...
            <div className="px-4 py-5 space-y-4">
              {['About', 'Features', 'Pricing', 'Contact'].map(item => <a key={item} href={`#${item.toLowerCase()}`} className="block px-4 py-2 text-gray-300 hover:text-white" onClick={() => setMobileMenuOpen(false)}>
                  {item}
                </a>)}
